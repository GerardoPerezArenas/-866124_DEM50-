    ...
    if (nc_validarDatosNumericosVacios() && nc_validarDatos()) {
                ...
                if (ajax.status === 200) {
                  ...
                  if (hijosFila[cont].childNodes[0] && hijosFila[cont].childNodes[0].nodeValue != "null") {
                    ...
                    if (codigoOperacion == "0") {
                        ...
                        // Intentar notificar a la ventana padre de forma segura
                        try {
                            if (self && self.parent && self.parent.opener && typeof self.parent.opener.retornoXanelaAuxiliar === 'function') {
                                console.log("Llamando a retornoXanelaAuxiliar en parent.opener");
                                self.parent.opener.retornoXanelaAuxiliar(lista);
                            } else if (window.opener && typeof window.opener.retornoXanelaAuxiliar === 'function') {
                                console.log("Llamando a retornoXanelaAuxiliar en window.opener");
                                window.opener.retornoXanelaAuxiliar(lista);
                            } else if (window.parent && typeof window.parent.retornoXanelaAuxiliar === 'function') {
                                console.log("Llamando a retornoXanelaAuxiliar en window.parent");
                                window.parent.retornoXanelaAuxiliar(lista);
                            } else {
                                console.log("retornoXanelaAuxiliar no disponible, continuando con cierre de ventana");
                            }
                        } catch(e) {
                            console.error("Error al llamar retornoXanelaAuxiliar:", e);
                        }
        ...
        jsp_alerta("A", nc_mensajeValidacion);
            ...
            function nc_obtenerComplementosPorTipo(callback) {
                console.log('=== FUNCIÓN nc_obtenerComplementosPorTipo VERSIÓN CORREGIDA v2+fallback ===');
                try {
                    ...
                    var attempts = [
                        { url: '<%=request.getContextPath()%>/PeticionModuloIntegracion.do', buildParams: function(){
                            ...
                            return 'tarea=preparar&modulo=MELANBIDE11&operacion=getComplementosPorTipo'
                        ...
                        { url: '<%=request.getContextPath()%>/servlet/MELANBIDE11', buildParams: function(){
                            return 'action=getComplementosPorTipo&dni='+encodeURIComponent(dni)+'&numExp='+encodeURIComponent(numExp);
                        }}
                    ];
                    ...
                    function tryRequest(index){
                        ...
                        console.log('[Complementos] Intento '+(index+1)+' URL:', attempt.url);
                        ...
                        xhr.onreadystatechange = function(){
                            if(xhr.readyState===4){
                                if(xhr.status===200){
                                    ...
                                    if(raw.length===0){
                                        console.warn('[Complementos] Respuesta vacía, reintentando con siguiente endpoint');
                                        tryRequest(index+1); return;
                                    }
                                    try{
                                        var response = JSON.parse(raw);
                                        if(typeof response.salariales==='undefined' || typeof response.extrasalariales==='undefined'){
                                            console.warn('[Complementos] JSON sin claves esperadas, fallback siguiente');
                                            tryRequest(index+1); return;
                                        }
                                        ...
                                        console.log('[Complementos] OK salariales:', complementosSalariales, 'extrasalariales:', complementosExtrasalariales);
                                        callback(complementosSalariales, complementosExtrasalariales);
                                    }catch(parseErr){
                                        console.error('[Complementos] Error parseando JSON:', parseErr, 'raw=', raw);
                                        tryRequest(index+1); return;
                                    }
                                } else {
                                    console.error('[Complementos] HTTP error', xhr.status, xhr.statusText);
                                    tryRequest(index+1); return;
                                }
                            }
                        };
            ...
            function nc_calcularRsbTotal() {
                try {
                    ...
                    nc_obtenerComplementosPorTipo(function(salariales, extrasalariales) {
                        ...
                        console.log("   ?? Salariales recibidos:", salariales, "(tipo:", typeof salariales, ")");
                        ...
                        console.log("   ?? Sustitución:", salBase, "+", pagExtra, "+", salariales);
                        ...
                        console.log("   ?? Complementos Extrasalariales (tipo 2):", extrasalariales);
